...
class Connect:
    ...
    def _get_unique_connections(
        self, system: Any, connect_file_path: Optional[Path] = None
    ) -> List[str]:
        """
        ...
        Args:
            system: System object containing models
            connect_file_path: Optional path to connect file (used to find caps directory)
            
        Returns:
            List of unique connection strings
        """
        # Determine caps directory
        caps_dir = connect_file_path.parent if connect_file_path else None
        if not caps_dir or not caps_dir.exists():
            LOG.debug("No caps directory available, using model.connect graph")
            return self._connections_from_model_graph(system)

        system_ids = set(system.get_models()) if system is not None else set()

        # Collect all caps files from caps_dir and subdirectories
        caps_map: Dict[float, Path] = {}
        
        # Include caps in current dir
        ...
        # Include caps in immediate subdirs (D/T/M/NC/DT/TD)
        for sub in caps_dir.iterdir():
            if sub.is_dir():
                for path in sub.glob("*.caps.pdb"):
                    ...
                    if system_ids and mid not in system_ids:
                        continue
                    # Prefer caps from subdirs if they exist
                    caps_map[mid] = path

        # Extract marker positions from all caps files
        ...
        for mid in mids:
            ...
            if partners:
                ...
                # Model has markers but no connections
                unique_connections.add(f"{int(mid)}.caps.pdb ; {model_type}")

        # Models without markers (self-connections only)
    ...
    def write_connect(self, system=None, connect_file=None):
        ...
        Args:
            system: System object containing models
            connect_file: Path to output connection file
        """
