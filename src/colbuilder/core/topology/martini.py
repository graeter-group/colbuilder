...
class Martini:
    ...
    def cap_pdb(self, pdb: Optional[List[str]] = None) -> Tuple[List[str], str, str]:
        ...
        try:
            ...
            last_residues: Dict[str, str] = {}
            for line in pdb:
                if line[0:6] in ("ATOM  ", "HETATM"):
                    cid = line[21:22]
                    if cid in self.is_chain:
                        last_residues[cid] = line[17:20].strip()

            all_nme = len(last_residues) >= 1 and all(r == "NME" for r in last_residues.values())
            if all_nme:
                cter = "none"
            elif len(pdb) > 2 and pdb[-2][17:20] == "CLA":
                cter = "CLA"
    ...
    def write_system_topology(
        ...
        try:
            with open(topology_file, "w") as f:
                ...
                ids_for_includes = (
                    processed_ids if processed_ids is not None else list(range(size))
                )
...
async def build_martini3(
    ...
    try:
        ...
        try:
            ...
            processed_int_ids = sorted({int(m) for m in processed_models})
            martini.write_system_topology(
                topology_file=final_topology_file,
                size=cnt_model,
                processed_ids=processed_int_ids,
            )
